...
class WebSearchAgent(BaseAgent):
    """Web Search Agent using Tavily WebSearchTool."""
    ...
    def execute(self, state: StartupState) -> StartupState:
        logger.info(
            f"WebSearchAgent collecting research snippets for concept: '{state.idea.idea_text}'"
        )

        try:
            results = self.search_tool.run_multi_query_search(
                query=state.idea.idea_text,
    ...
    # Required by tests and LangGraph
