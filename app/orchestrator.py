...
from state.schema import StartupIdea, AgentState
...
class ApplicationOrchestrator:
    ...
    def ask_advisor(self, session_id: str, user_question: str, chat_history: List[Dict[str, str]]) -> str:
        state = self.memory.get_state(session_id)
        if not state:
            return f"Session '{session_id}' not found. Please run a validation first."
        return self.advisor.answer_question(user_question, state, chat_history)

    def get_session_history(self, session_id: str) -> Optional[AgentState]:
